...
function nearSearch() {
  ...
  if (rangeActivate) {
    ...
    function clusterInfo(places) {
      ...
      let clusterElement = places.getMarkers();
      let contentList = '';
      // 출력 String임 for 써서 이어붙이기
      for (i in clusterElement) {
        contentList += '<div class="placeinfo">' +
          '   <a class="title" href="' + clusterElement[i].place_url + '" target="_blank" title="' + clusterElement[i].place_name + '">'
          + clusterElement[i].place_name + '</a></div>';
      }
      console.log(places.getMarkers());
      console.log(clusterElement);
      contentNode.innerHTML = contentList;
      placeOverlay.setPosition(new kakao.maps.LatLng(places.y, places.x));
      placeOverlay.setMap(map);
